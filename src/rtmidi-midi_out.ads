...
package Rtmidi.Midi_Out is
   ...
   procedure Open_Port
     (Self : in out Midi_Out'Class; Number : Natural := 0;
      Name :        String := "RtMidi Output") with
     Pre => Self.Valid;

   procedure Open_Virtual_Port
     (Self : in out Midi_Out'Class; Name : String := "RtMidi Output") with
     Pre => Self.Valid;

   procedure Close_Port (Self : in out Midi_Out'Class) with
     Pre => Self.Valid;

   function Get_Port_Count (Self : Midi_Out'Class) return Natural with
     Pre => Self.Valid;

   function Get_Port_Name
     (Self : Midi_Out'Class; Number : Natural) return String with
     Pre => Self.Valid;
   ...
   function Error_Message (Self : Midi_Out'Class) return String with
     Pre => Self.Valid;
...
private
   ...
   procedure Free (Self : in out Midi_Out'Class) with
     Pre => Self.Valid;

   overriding procedure Finalize (Self : in out Midi_Out);
